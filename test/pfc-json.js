...
describe('pfc-json', () => {
    ...
    it('object', () => {
        assert.equal(compile(JSON.stringify({}), 'pfc', {
            pureMiddleCode: true
        }), 'sys_data(sys_object(sys_void()))');

        assert.equal(compile(JSON.stringify({
            a: 1
        }), 'pfc', {
            pureMiddleCode: true
        }), 'sys_data(sys_object(sys_pair(sys_string("a"), sys_number("1"))))');
        ...
        assert.equal(compile(JSON.stringify({
            ...
            pureMiddleCode: true
        }), 'sys_data(sys_object(sys_pair(sys_pair(sys_string("a"), sys_number("1")), sys_pair(sys_string("b"), sys_string("2")))))');
    ...
    it('array', () => {
        assert.equal(compile(JSON.stringify([]), 'pfc', {
            pureMiddleCode: true
        }), 'sys_data(sys_array(sys_void()))');
        assert.equal(compile(JSON.stringify([1]), 'pfc', {
            pureMiddleCode: true
        }), 'sys_data(sys_array(sys_number("1")))');
        assert.equal(compile(JSON.stringify([1, 2]), 'pfc', {
            pureMiddleCode: true
        }), 'sys_data(sys_array(sys_pair(sys_number("1"), sys_number("2"))))');
    });
});
